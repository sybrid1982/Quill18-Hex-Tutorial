    ...
    public HexPath(HexMap hexMap, Hex start, Hex end)
    {
        ...
        while (openSet.Count > 0)
        {
            ...
            foreach (Path_Edge<Hex> e in current.edges)
            {
                ...
                {
                    //then we've found a path to a new hex and need to add it to the open set
                    openSet.Enqueue(e.node, fScore[e.node]);
                } else
                {
                    //Otherwise this is merely a faster way to a hex we've already had and can update its score
